...
internal class RegistrationUserViewModel @Inject constructor(
    ...
    private val _responseStudents = MutableStateFlow<PagingData<Student>>(PagingData.empty())
    ...
    private fun registrationHeadman(body: HeadmanRegistrationBody) {
        viewModelScope.launch {
            _responseResultRegistration.value = headmanRepository.registration(body)
        }
    }

    private fun registrationHeadmanDeputy(body: HeadmanRegistrationBody) {
        viewModelScope.launch {
            _responseResultRegistration.value = headmanRepository.registrationDeputy(body)
        }
    }

    private fun registrationStudent(body: StudentRegistrationBody) {
        viewModelScope.launch {
